...
async function uploadCroppedImage() {
  ...
  if (Math.abs(outputAspect - stageAspect) > 0.01) {
    setCropSize(outputWidth, outputWidth / stageAspect);
  ...
  const sx = (-left / displayWidth) * cropState.image.naturalWidth;
  const sy = (-top / displayHeight) * cropState.image.naturalHeight;
  const sw = (rect.width / displayWidth) * cropState.image.naturalWidth;
  const sh = (rect.height / displayHeight) * cropState.image.naturalHeight;
  const sourceAspect = sw / sh;
  const cropWidth = Math.round(Math.max(1, Math.min(cropState.image.naturalWidth, sw)));
  const cropHeight = Math.round(cropWidth / sourceAspect);
  ...
  cropCtx.drawImage(cropState.image, sx, sy, sw, sh, 0, 0, cropWidth, cropHeight);
  ...
  ctx.drawImage(cropCanvas, 0, 0, cropWidth, cropHeight, 0, 0, outputWidth, outputHeight);
